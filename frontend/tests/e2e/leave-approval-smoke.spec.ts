...
test.describe("휴가·전자결재 운영 스모크", () => {
  ...
  test("권한에 맞는 휴가 운영 메뉴를 표시한다", async ({ page }) => {
    await page.getByRole("navigation").getByRole("button", { name: "전자결재" }).click();

    if (expectLeaveAdmin) {
      await expect(page.getByRole("button", { name: "대체휴무 관리" })).toBeVisible();
      await page.getByRole("button", { name: "대체휴무 관리" }).click();
      ...
      await expect(page.getByRole("button", { name: "대체휴무 관리" })).toHaveCount(0);
      await expect(page.getByRole("button", { name: "휴가정책" })).toHaveCount(0);
