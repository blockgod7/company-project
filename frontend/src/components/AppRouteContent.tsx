...
const PersonalCalendarPage = lazy(() => import("../pages/PersonalCalendarPage").then((module) => ({ default: module.PersonalCalendarPage })));
...
type AppRouteContentProps = {
  ...
  openApproval: (approvalId: number) => void;
};

export function AppRouteContent({ route, user, isAdmin, canViewPreview, approvalLaunch, plannedFeatureCode, globalSearch, navigate, openApprovals, openApproval }: AppRouteContentProps) {
  return (
    <main className="content">
      <Suspense fallback={<div className="route-loading" role="status">화면을 불러오는 중입니다…</div>}>
        ...
        {route === "calendar" && <PersonalCalendarPage onOpenApproval={openApproval} />}
