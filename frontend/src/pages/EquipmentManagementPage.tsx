...
import type { DeptNode, Employee, Equipment, EquipmentHistoryEvent, EquipmentProcess, EquipmentReport, PageResponse, User } from "../types";

const today = () => new Date().toISOString().slice(0, 10);
const flattenDepts = (nodes: DeptNode[]): DeptNode[] => nodes.flatMap((node) => [node, ...flattenDepts(node.children)]);
const emptyReport = () => ({ equipmentId: "", title: "", symptom: "", requestContent: "", priority: "NORMAL", occurredOn: today() });
...
export function EquipmentManagementPage({ user, isAdmin }: { user: User; isAdmin: boolean }) {
  ...
  const [completion, setCompletion] = useState({ workResult: "", causeAnalysis: "", actionTaken: "" });
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");

  const canAssign = user.roleCode === "ADMIN" || user.roleCode === "MANAGER";

  async function load() {
    try {
      const [nextEquipment, nextReports, employeePage, nextProcesses, nextDeptTree] = await Promise.all([
      ...
      setEquipment(nextEquipment); setReports(nextReports); setEmployees(employeePage.content); setProcesses(nextProcesses); setDeptTree(nextDeptTree);
  ...
  async function submitCompletion(event: React.FormEvent) {
    ...
    try {
      ...
      setMessage("작업결과를 등록하고 완료 결재를 요청했습니다."); setCompletion({ workResult: "", causeAnalysis: "", actionTaken: "" }); await load();
      ...
      <section className="equipment-main">
        ...
        <div className="equipment-card">
          ...
          <form className="equipment-form" onSubmit={submitReport}>
            <label>설비<select required value={reportForm.equipmentId} onChange={(e) => setReportForm({ ...reportForm, equipmentId: e.target.value })}><option value="">설비를 선택하세요</option>{equipment.map((item) => <option key={item.equipmentId} value={item.equipmentId}>{item.equipmentNo} · {item.equipmentName}</option>)}</select></label>
            ...
            <button className="primary wide" type="submit"><Send size={16} /> 부서장 결재 요청</button>
          </form>
        </div>
        <div className="equipment-card"><div className="panel-head"><h2><Wrench size={19} /> 진행 중인 이상보고</h2><button type="button" onClick={() => void load()}>새로고침</button></div>
...
function ReportDetail({ report, user, canAssign, employees, assignment, setAssignment, completion, setCompletion, history, onAssign, onComplete, onUpload }: { report: EquipmentReport; user: User; canAssign: boolean; employees: Employee[]; assignment: { assigneeEmpId: string; plannedStartOn: string; plannedEndOn: string; instruction: string }; setAssignment: (value: { assigneeEmpId: string; plannedStartOn: string; plannedEndOn: string; instruction: string }) => void; completion: { workResult: string; causeAnalysis: string; actionTaken: string }; setCompletion: (value: { workResult: string; causeAnalysis: string; actionTaken: string }) => void; history: EquipmentHistoryEvent[]; onAssign: (event: React.FormEvent) => Promise<void>; onComplete: (event: React.FormEvent) => Promise<void>; onUpload: (file: File) => Promise<void> }) {
