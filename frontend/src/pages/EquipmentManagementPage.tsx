...
import type { DeptNode, Employee, Equipment, EquipmentAssignmentAuthority, EquipmentAssignmentPermission, EquipmentHistoryEvent, EquipmentMigrationIssueSummary, EquipmentMigrationRun, EquipmentProcess, EquipmentReport, PageResponse, User } from "../types";
...
export function EquipmentManagementPage({ user, isAdmin }: { user: User; isAdmin: boolean }) {
  ...
  const [migrationRuns, setMigrationRuns] = useState<EquipmentMigrationRun[]>([]);
  const [migrationIssueSummary, setMigrationIssueSummary] = useState<EquipmentMigrationIssueSummary[]>([]);
  const [selectedMigrationRunId, setSelectedMigrationRunId] = useState<number | null>(null);
  ...
  async function load() {
    try {
      const [nextEquipment, activeReportPage, terminalReportPage, employeePage, nextProcesses, nextDeptTree, nextPermission] = await Promise.all([
        api<Equipment[]>("/equipment"),
        api<PageResponse<EquipmentReport>>("/equipment/reports/paged?terminal=false&page=0&size=500"),
        api<PageResponse<EquipmentReport>>("/equipment/reports/paged?terminal=true&page=0&size=200"),
      ...
      const nextReports = [...activeReportPage.content, ...terminalReportPage.content];
      ...
      setHistoryPage(terminalReportPage.page); setHistoryTotalPages(terminalReportPage.totalPages);
      ...
      if (isAdmin) setMigrationRuns(await api<EquipmentMigrationRun[]>("/equipment/migrations")); else setMigrationRuns([]);
  ...
  async function openReport(report: EquipmentReport) {
    ...
    setCompletion({
      ...
      completedOn: report.completedOn ?? today(),
      workDurationHours: report.workDurationHours?.toString() ?? "",
      approverEmpIds: [],
    });
  ...
  async function openMigrationRun(runId: number) {
    if (selectedMigrationRunId === runId) {
      setSelectedMigrationRunId(null);
      setMigrationIssueSummary([]);
      return;
    }
    try {
      setSelectedMigrationRunId(runId);
      setMigrationIssueSummary(await api<EquipmentMigrationIssueSummary[]>(`/equipment/migrations/${runId}/issues/summary`));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "이식 검증 결과를 불러오지 못했습니다.");
    }
  }

  async function loadMoreHistory() {
    const nextPage = historyPage + 1;
    if (nextPage >= historyTotalPages) return;
    try {
      const next = await api<PageResponse<EquipmentReport>>(`/equipment/reports/paged?terminal=true&page=${nextPage}&size=200`);
      setReports((current) => {
        const existingIds = new Set(current.map((item) => item.reportId));
        return [...current, ...next.content.filter((item) => !existingIds.has(item.reportId))];
      });
      setHistoryPage(next.page);
      setHistoryTotalPages(next.totalPages);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "추가 설비이력을 불러오지 못했습니다.");
    }
  }
  ...
  const terminalStates = new Set(["COMPLETED", "CANCELLED", "REJECTED"]);
  const inProgressReports = reports.filter((report) => !terminalStates.has(report.state));
  const completedReports = reports.filter((report) => terminalStates.has(report.state));
...
function ReportDetail({ report, user, canAssign, employees, assignment, setAssignment, completion, setCompletion, history, onAssign, onComplete, onUpload }: { report: EquipmentReport; user: User; canAssign: boolean; employees: Employee[]; assignment: { assigneeEmpId: string; plannedStartOn: string; plannedEndOn: string; instruction: string }; setAssignment: (value: { assigneeEmpId: string; plannedStartOn: string; plannedEndOn: string; instruction: string }) => void; completion: { workResult: string; causeAnalysis: string; actionTaken: string; completedOn: string; workDurationHours: string; approverEmpIds: number[] }; setCompletion: (value: { workResult: string; causeAnalysis: string; actionTaken: string; completedOn: string; workDurationHours: string; approverEmpIds: number[] }) => void; history: EquipmentHistoryEvent[]; onAssign: (event: React.FormEvent) => Promise<void>; onComplete: (event: React.FormEvent) => Promise<void>; onUpload: (file: File) => Promise<void> }) {
  const canSubmitCompletion = report.assigneeEmpId === user.empId && (
    report.state === "IN_PROGRESS"
    || report.state === "REWORK"
    || (report.state === "PENDING_COMPLETION_APPROVAL" && report.completionApprovalId === null)
  );
  return <div className="equipment-card report-detail"><div className="panel-head"><h2>{report.title}</h2><span className={`equipment-state ${report.state.toLowerCase()}`}>{stateLabel(report.state)}</span></div><dl><div><dt>설비</dt><dd>{report.equipmentNo} · {report.equipmentName}</dd></div><div><dt>신고자</dt><dd>{report.reporterName}</dd></div><div><dt>증상</dt><dd>{report.symptom}</dd></div><div><dt>요청</dt><dd>{report.requestContent}</dd></div>{report.state === "CANCELLED" && <><div><dt>취소 단계</dt><dd>{cancelStageLabel(report.cancelStage)}</dd></div><div><dt>취소자</dt><dd>{report.cancelledByName ?? "기록 없음"}</dd></div><div><dt>취소일</dt><dd>{report.cancelledOn ?? "기록 없음"}</dd></div></>}</dl><label className="file-upload"><Upload size={15} /> 선택 첨부<input type="file" onChange={(e) => e.target.files?.[0] && void onUpload(e.target.files[0])} /></label>
    {canAssign && (report.state === "ASSIGNMENT_PENDING" || (report.state === "IN_PROGRESS" && report.assigneeEmpId === null)) && <form className="equipment-form compact" onSubmit={(e) => void onAssign(e)}><h3>{report.state === "IN_PROGRESS" ? "진행 업무 담당자 연결" : "작업 배분"}</h3><label>보전 담당자<select required value={assignment.assigneeEmpId} onChange={(e) => setAssignment({ ...assignment, assigneeEmpId: e.target.value })}><option value="">선택</option>{employees.map((employee) => <option value={employee.empId} key={employee.empId}>{employee.empName} · {employee.deptName}</option>)}</select></label><label>시작일<input type="date" value={assignment.plannedStartOn} onChange={(e) => setAssignment({ ...assignment, plannedStartOn: e.target.value })} /></label><label>종료일<input type="date" value={assignment.plannedEndOn} onChange={(e) => setAssignment({ ...assignment, plannedEndOn: e.target.value })} /></label><label>작업지시<textarea value={assignment.instruction} onChange={(e) => setAssignment({ ...assignment, instruction: e.target.value })} /></label><button className="primary" type="submit">{report.state === "IN_PROGRESS" ? "담당자 연결" : "작업 배분"}</button></form>}
    {canSubmitCompletion && <form className="equipment-form compact" onSubmit={(e) => void onComplete(e)}><h3>{report.state === "PENDING_COMPLETION_APPROVAL" ? "완료 결재 다시 요청" : "작업완료 보고"}</h3><label>완료 일자<input required type="date" value={completion.completedOn} onChange={(e) => setCompletion({ ...completion, completedOn: e.target.value })} /></label><label>소요 시간(시간)<input required type="number" min="0.01" step="0.01" value={completion.workDurationHours} onChange={(e) => setCompletion({ ...completion, workDurationHours: e.target.value })} placeholder="예: 2.5" /></label><label>작업 결과<textarea required value={completion.workResult} onChange={(e) => setCompletion({ ...completion, workResult: e.target.value })} /></label><label>원인 분석<textarea value={completion.causeAnalysis} onChange={(e) => setCompletion({ ...completion, causeAnalysis: e.target.value })} /></label><label>조치 내용<textarea required value={completion.actionTaken} onChange={(e) => setCompletion({ ...completion, actionTaken: e.target.value })} /></label><button className="primary" type="submit"><CheckCircle2 size={16} /> 완료 결재 요청</button></form>}
    {canSubmitCompletion && <EmployeeMultiPicker title="완료 결재자" user={user} employees={employees} selectedIds={completion.approverEmpIds} disabledIds={[user.empId]} ordered onChange={(approverEmpIds) => setCompletion({ ...completion, approverEmpIds })} />}
...
function stateLabel(value: string) { return ({ DRAFT: "작성 중", PENDING_INITIAL_APPROVAL: "최초 결재 대기", ASSIGNMENT_PENDING: "배분 대기", IN_PROGRESS: "작업 진행", PENDING_COMPLETION_APPROVAL: "완료 결재 대기", REWORK: "재작업", COMPLETED: "완료", CANCELLED: "취소", REJECTED: "반려" } as Record<string, string>)[value] ?? value; }
function statusLabel(value: string) { return ({ IN_USE: "사용중", UNDER_MAINTENANCE: "정비중", RENTED: "임대", DISPOSED: "폐기", LONG_TERM_STORAGE: "장기보관" } as Record<string, string>)[value] ?? value; }
function cancelStageLabel(value: EquipmentReport["cancelStage"]) { return value ? ({ REQUEST: "요청 단계", ASSIGNMENT: "업무 배분 단계", WORK: "작업 단계" } as Record<string, string>)[value] ?? value : "기록 없음"; }
function migrationIssueLabel(value: string) { return ({ REQUEST_USER_NOT_MAPPED: "요청자 미연결", WORK_USER_NOT_MAPPED: "작업자 미연결", CANCEL_USER_NOT_MAPPED: "취소자 미연결", ACTIVE_ASSIGNEE_REVIEW_REQUIRED: "진행 업무 담당자 확인 필요", WORK_ORDER_EQUIPMENT_REQUIRED: "설비번호 없는 업무", TARGET_EQUIPMENT_ALREADY_EXISTS: "기존 설비번호 중복" } as Record<string, string>)[value] ?? value; }
